...
def f_maj(x, y, z):
    return ((not x) and y and z) or (x and (not z))
...
def xor_polynom(tt):
    ...
    # Первая строка треугольника - сама таблица истинности
    triangle.append(tt_line.copy())
    
    # Строим остальные строки треугольника
    for i in range(1, 8):
        new_line = []
        for j in range(len(triangle[i-1]) - 1):
            new_line.append(triangle[i-1][j] ^ triangle[i-1][j+1])
        triangle.append(new_line)
    
    # Выводим треугольник для отладки
    for line in triangle:
        print(line)
    
    # Собираем полином из первых элементов каждой строки треугольника
    i = 0
    for A in range(2):
        for B in range(2):
            for C in range(2):
